...
const ARGOCD_TARGETS = [
  { key: "prod", name: "ArgoCD Prod" },
  { key: "test", name: "ArgoCD Test" },
];
...
function App() {
  ...
  const fetchApps = () => {
    ...
    setLoading(true);
    fetch(`/api/apps?target=${selectedTarget}`)
      .then((res) => {
  ...
  // Sayfa, filtre değişince ilk sayfaya döner
  useEffect(() => {
    setCurrentPage(1);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500">
        <div className="bg-white/90 rounded-2xl shadow-xl p-10 flex flex-col items-center gap-8 min-w-[350px]">
          <h1 className="text-3xl font-bold text-gray-800 mb-2 text-center">
            ArgoCD Sunucusu Seçin
          </h1>
          <div className="flex flex-col gap-4 w-full">
            {ARGOCD_TARGETS.map(target => (
              <button
                key={target.key}
      ...
      <div className="w-full max-w-5xl bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl p-10 flex flex-row gap-6">
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex items-center gap-4 mb-2">
            <h1 className="text-4xl font-extrabold text-gray-900 text-center tracking-tight drop-shadow flex-1">
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-pink-500">
                ArgoCD Apps
              </span>
            </h1>
            <div className="px-3 py-1 rounded-xl font-semibold bg-indigo-100 text-indigo-700 shadow text-sm">
              {
                ARGOCD_TARGETS.find(a => a.key === selectedTarget)?.name || selectedTarget
              }
            </div>
            {/* ORTAM DEĞİŞTİR butonu sağda! */}
            <button
              onClick={() => setSelectedTarget(null)}
              className="px-4 py-1 ml-4 rounded-lg bg-pink-100 text-pink-700 font-semibold shadow hover:bg-pink-200 transition-all"
            >
              Ortam Değiştir
            </button>
          </div>
